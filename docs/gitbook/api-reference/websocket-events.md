...
QoreChain provides real-time event streaming through two WebSocket interfaces: the EVM-compatible WebSocket and the Consensus Engine Engine RPC WebSocket.
...
The RPC WebSocket uses the Consensus Engine Engine event subscription system. Clients subscribe with a query string that filters events by type and attributes.
