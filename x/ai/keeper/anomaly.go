...
func (h *HeuristicAnomaly) DetectAnomaly(_ context.Context, tx types.TransactionInfo, history []types.TransactionInfo) (*types.AnomalyResult, error) {
	...
	// Single pass over history to collect all sender-related stats at once.
	var senderCount, sameReceiverCount int
	var amtSum, amtSumSq float64
	var amtCount int
	for _, htx := range history {
		if htx.Sender == tx.Sender {
			senderCount++
			if htx.Receiver == tx.Receiver {
				sameReceiverCount++
			}
			if htx.Amount > 0 {
	...
	if senderCount > h.maxTxPerMinute {
	...
	if amtCount > 2 {
		mean := amtSum / float64(amtCount)
		variance := (amtSumSq / float64(amtCount)) - (mean * mean)
		if variance > 0 {
	...
	if senderCount < 3 && tx.Amount > 1_000_000_000 { // < 3 txs and > 1000 QOR
